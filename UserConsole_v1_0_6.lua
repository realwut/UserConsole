...
function console_exec(input)
	...
	if (args[1]:upper() == "FULLBRIGHT") or (args[1]:upper() == "FB") then
		game.Lighting.OutdoorAmbient = Color3.new(1, 1, 1)
		game.Lighting.FogEnd = 99999999
		game.Lighting.GlobalShadows = false
	end
	
	if (args[1]:upper() == "FLING") then
		if (Player.Character.HumanoidRootPart:FindFirstChild("BodyThrust") == nil) then
			...
			if not (args[2] == nil) then
				bambam.Force = Vector3.new(args[2],0,0)
			else
				bambam.Force = Vector3.new(75000,0,0)
			end
	...
	if (args[1]:upper() == "EXEC") then
		if (args[2]:upper() == "RSPY") then
		...
		if Fly_Enabled then
            ...
            if (Console_Active == false) and (Player.PlayerGui.Chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar:IsFocused() == false) then
                ...
                if UserInputService:IsKeyDown(Enum.KeyCode.W) then
                    vel_x += Camera.CFrame.LookVector.X*Fly_Speed
                    vel_z += Camera.CFrame.LookVector.Z*Fly_Speed
                end
                ...
                if UserInputService:IsKeyDown(Enum.KeyCode.A) then
                    vel_x += Camera.CFrame.LookVector.Z*Fly_Speed
                    vel_z -= Camera.CFrame.LookVector.X*Fly_Speed
                end
...
function showcredits()
	if Showing_Credits == false then
		...
		NotifTitle.Text = "UserConsole v1.0.5"
